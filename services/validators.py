...
def search_business_name(data):
    # search database for name, address and email and see if there is an existing match anywhere.
    businesses = all_businesses()
    
    for business in businesses:
        if business['name'].lower() == data['name']:
            if business['address'].lower() == data['address']:
                if business['email'].lower() == data['email']:
                    if business['telephone'] == data['telepehone']:
                        return False
    return True